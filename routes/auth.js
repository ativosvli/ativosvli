...
router.get('/usuarios', (req, res) => {
  ...
  const db = getDatabase();
  const usuarios = db.prepare('SELECT id, username, nome, perfil, ativo, created_at FROM usuarios ORDER BY id').all();
  res.json(usuarios);
});
...
router.post('/register', (req, res) => {
  ...
  db.prepare('INSERT INTO usuarios (username, password, nome, perfil) VALUES (?, ?, ?, ?)').run(username, hash, nome, 'admin');
